...
while reader.has_time_blocks()
    ...
    if time_block.isEventTimeBlock()
        ...
        for event_idx = 1:length(time_block.value.prompt_events)
            ...
            mod_and_el = petsird.helpers.get_module_and_element(header.scanner.scanner_geometry, event.detection_bins("    [ModuleAndElement(module=%d, el=%d), ModuleAndElement(module=%d, el=%d)]\n", ...
